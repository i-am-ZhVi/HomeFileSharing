...
async def get_list(sub_name: str | None = None, extension_id: int | None = None, mime_type_id: int | None = None, session: AsyncSession = Depends(get_db_session)):
    repo = SQLAlchemyFileRepository(session)
    usecase = GetFileListUseCase(repo)
    response = await usecase.execute(sub_name=sub_name, extension_id=extension_id, mime_type_id=mime_type_id)
...
async def create(name: str, extension_id: int, mime_type_id: int, password_hash: str | None, session: AsyncSession = Depends(get_db_session)):
    repo = SQLAlchemyFileRepository(session)
    usecase = CreateFileUseCase(repo)
    response = await usecase.execute(name=name, extension_id=extension_id, mime_type_id=mime_type_id, password_hash=password_hash)

    return FileResponse.model_validate(response, from_attributes=True)


@router.patch("/{id}", response_model=FileResponse | None)
async def update(id: int, name: str | None = None, extension_id: int | None = None, mime_type_id: int | None = None, password_hash: str | None = None, session: AsyncSession = Depends(get_db_session)):
    repo = SQLAlchemyFileRepository(session)
    usecase = UpdateFileUseCase(repo)
    response = await usecase.execute(file_id=id, name=name, extension_id=extension_id, mime_type_id=mime_type_id, password_hash=password_hash)
